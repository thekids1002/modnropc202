...
public class GameScr : mScreen, IChatable
{
	...
	public static bool hsme;
	public static bool alogin;
	public static int akhu = -1;
	...
	public static void autoDanhPet()
    {
		for(int i = 0;  i < GameScr.vMob.size(); i++)
        {
			Mob mob = (Mob) GameScr.vMob.elementAt(i);
			int distance = Math.abs(Char.myCharz().cx - mob.x);
			int distance2 = Math.abs(Char.myCharz().cy - mob.y);
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num3; i++)
				{
					if (Main.isPC)
					{
						...
						if (TField.isQwerty)
						{
							string[] obj = new string[8] { "1", "2", "3", "4", "5", "6", "7", "8"};
							array2 = obj;
							obj[4] = "5";
						}
						else
	...
	public void actionPerform(int idAction, object p)
	{
		...
		{
			Char char2 = (Char)p;
			Service.gI().friend(1, char2.charID);
			break;
		}
		case 11113:
		{
	...
	public void chatVip(string chatVip)
	{
        ...
        {
			string mychatvip = chatVip;
			mychatvip = mychatvip.Replace("BOSS", "");
			mychatvip = mychatvip.Replace("vừa xuất hiện tại", "-");
			long time_ago = mSystem.currentTimeMillis();
			hoangkiet.myChatVip.addElement(mychatvip + "|" +time_ago);
		}
		if (!startChat)
		{
	...
	static GameScr()
	{
		...
		Filelog = "TextData/Log.txt";
		FileAccount = "TextData/Account.txt";
	}

	public static void Loadfileacc()
	{
		if (File.Exists(Filelog))
		{
			string[] array = File.ReadAllText(Filelog).Split('|');
			ID = int.Parse(array[0]);
			Account = array[1];
			Password = array[2];
			Server = int.Parse(array[3]) - 1;
			File.Delete(Filelog);
		}
	}
