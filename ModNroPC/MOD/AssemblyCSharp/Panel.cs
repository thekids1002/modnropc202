...
public class Panel : IActionListener, IChatable
{
	...
	private void paintShop(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (item != null)
			{
				...
				{
					mFont.tahoma_7b_green.drawString(g, "["+item.template.id +"]" + item.template.name + text, num2 + 5, num3 + 1, 0);
				}
				else
				{
					mFont.tahoma_7_green2.drawString(g, "[" + item.template.id + "]" + item.template.name + text, num2 + 5, num3 + 1, 0);
				}
				string text2 = string.Empty;
	...
	private void paintPetInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrPetSkill.Length; i++)
		{
			...
			if (item != null)
			{
				...
				mFont.tahoma_7_green2.drawString(g, text + "[" + item.template.id + "]" + item.template.name + text, num3 + 5, num4 + 1, 0);
				string text2 = string.Empty;
				if (item.itemOption != null)
				{
	...
	private void paintBox(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			mFont.tahoma_7_green2.drawString(g, text + "[" + item.template.id + "]" + item.template.name, num2 + 5, num3 + 1, 0);
			string text2 = string.Empty;
			if (item.itemOption != null)
			{
				if (item.itemOption.Length > 0 && item.itemOption[0] != null)
	...
	private void paintCombine(mGraphics g)
	{
		...
		for (int j = 0; j < vItemCombine.size() + 1; j++)
		{
			...
			mFont.tahoma_7_green2.drawString(g, "[" + item.template.id + "]" + item.template.name + text, num + 5, num2 + 1, 0);
			string text2 = string.Empty;
			if (item.itemOption != null)
			{
				if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
	...
	private void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			mFont.tahoma_7_green2.drawString(g, "[" + item.template.id + "]" + item.template.name + text, num3 + 5, num4 + 1, 0);
			string text2 = string.Empty;
			if (item.itemOption != null)
			{
				if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
