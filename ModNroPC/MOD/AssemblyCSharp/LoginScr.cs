...
public class LoginScr : mScreen, IActionListener
{
	...
	{
		GameCanvas.loginScr.tfPass.setText(GameScr.Password);
		GameCanvas.loginScr.tfUser.setText(GameScr.Account);
		GameCanvas.connect();
		Service.gI().login(GameScr.Account, GameScr.Password, GameMidlet.VERSION, 0);
		if (Session_ME.connected)
		{
		
			GameCanvas.closeKeyBoard();
			GameScr.isLogin = true;
		}
		else
		{
