...
public class GameCanvas : IActionListener
{
	...
	{	
		if(info.ToLower().Trim().Equals("không thể đổi khu vực trong map này"))
        {
			return;
        }
		closeKeyBoard();
        ...
        try
        {
            while (true)
            {
				...
				if (ServerListScreen.testConnect == 2)
				{
					...
					loginScr.doLogin2();
					Thread.Sleep(5000);
					Char.myCharz().cy = Char.myCharz().cy - 1;
					Service.gI().charMove();
					while (TileMap.zoneID != GameScr.akhu && GameScr.akhu != -1)
					{
						Service.gI().requestChangeZone(GameScr.akhu, 1);
						Thread.Sleep(5000);
					}
					return;
